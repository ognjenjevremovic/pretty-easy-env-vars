...
function setFilepath(param) {
    ...
    switch (typeof param) {
        case 'string':
            ...
            break;
        case 'object':
            //  Set defaults if fileName is not defined
            if (prop.fileName === undefined || prop.fileName === null) {
            ...
            //  fileName set
            if (typeof prop.fileName === 'string') {
                //  Split by dot
                _fileName = prop.fileName.split('.');
                //  File extension included
                if (_fileName.length > 1) {
                    _fileName.forEach(function(part, _index) {
                        //  Set file extension
                        if (_index === (_fileName.length - 1)) {
                            _fileExt =  part;
                        }
                    });
                }
